...
return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:text-white dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl transition-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
        <img src={theme === 'dark' ? assets.darkMode : assets.logo} alt="Image here" className='w-full max-w-32' width="100px"/>
            ...
            <img src={assets.loop} className='w-4 not-dark: invert' alt="" />
            ...
            <img src="" className='hidden group-hover:block w-4 cursor-pointer not-dark:invert' alt="bin" />
            </div>
            ))
        }
        </div>
{/* Dark Mode Toggle */}
<div className='flex items-center gap-2 p-3 mt-4 justify-between border border-gray-300 dark:border-white/15 rounded-md'>
...
<img src={assets.user} className='w-7 rounded-full' alt="" />
<p className='flex-1 text-sm dark: text-primary truncate'>{user ? user.name
:'Login your account'}</p>
{user && <img src={assets.logout} className='h-5 cursor-pointer hidden dark:invert not-dark:invert-0 group-hover:block'/>}
</div>




<img onClick={() => setIsMenuOpen(false)} src={assets.logout} className='absolute top-3 right-3 w-5 h-5 cursor-pointer md:hidden dark:invert not-dark:invert-0' alt="" />
